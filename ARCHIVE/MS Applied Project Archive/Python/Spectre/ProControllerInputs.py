...
XInput.set_deadzone(XInput.DEADZONE_LEFT_THUMB, 8689)
XInput.set_deadzone(XInput.DEADZONE_RIGHT_THUMB, 8689)

maxLin = 15 # maximum desired linear speed in ft/s
maxRot = 2*np.pi # maximum desired rotational speed in rad/s

def get_sticks():
    state = XInput.get_state(0) # get the state of the whole controller
    # read stick values into an array, multiply by 1000, and convert to an int
