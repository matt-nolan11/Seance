...
Geometric parameters are specific to the mini prototype version of Spectre
'''
...
sigma = np.pi/180 * np.array([32.5319, 32.5319, 33.5217, 33.5217]) # acute angles of wheel position lines w.r.t. robot x-axis, in rad
beta = np.pi/180 * np.array([35, 35, 45, 45])  # acute angles of wheels w.r.t robot y-axis, in rad
r_g = np.array([[0], [0.95], [0]])  # vector from C to G, in inches
d = [4.0365, 4.0365, 3.9975, 3.9975]  # distance from C to the center of each wheel, in inches
radius = 1  # wheel radii (in inches, assumed to be the same for each wheel)

# Relevant vectors from above constants
r = []  # vectors from G to the center of each wheel
for i in range(4):
...
v = []  # vectors representing the direction each wheel is "pointed"
for i in range(4):
